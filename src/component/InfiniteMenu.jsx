      ...
      {activeItem && (
        <div
          className={[
            ...
            "max-w-[min(92vw,720px)] w-[92vw] sm:w-[min(92vw,720px)]",
            "rounded-2xl border border-white/20 bg-white/70 backdrop-blur-md shadow-xl",
            "px-4 py-3 sm:px-6 sm:py-4",
            "transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)]",
            isMoving
              ? "opacity-0 translate-y-3 pointer-events-none duration-100"
        ...
        >
          <div className="flex items-center gap-3 sm:gap-4">
            {/* 썸네일: 단색 박스나 아이콘 */}
            <div
              className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl border border-black/5"
              style={{
                backgroundImage: activeItem.image?.startsWith("data:")
                  ? undefined
                  : `url(${activeItem.image})`,
                backgroundColor: activeItem.image?.startsWith("data:")
                  ? "#ddd"
                  : undefined,
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate font-semibold text-slate-900 text-[clamp(16px,2.2vw,22px)] leading-tight">
                {activeItem.title || "제목 없음"}
              </div>
              <div className="mt-0.5 text-slate-700/90 text-[clamp(12px,1.8vw,16px)] leading-snug line-clamp-2">
                {activeItem.description || "설명이 없습니다."}
              </div>
            </div>
            ...
            {activeItem.link && (
              <button
                ...
                className="shrink-0 rounded-lg px-3 py-2 text-[14px] font-medium bg-black text-white hover:bg-black/90 transition-colors"
              >
                열기
              </button>
            )}
