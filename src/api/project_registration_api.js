...
// Submit distribution request for a project
export const submitDistributionRequest = async ({ projectId, document = [], image = [], distributionAmount, distributionSummary }) => {
    try {
        // Filter out empty strings from arrays
        const cleanDocument = document.filter(doc => doc && doc.trim() !== '');
        const cleanImage = image.filter(img => img && img.trim() !== '');
        
        const response = await privateApi.post('/product/request/distribution', {
            projectId: projectId,
            document: cleanDocument,
            image: cleanImage,
        ...
        console.error('Distribution request API Error:', error);
        ...
        if (error.response?.status === 400) {
            ...
            throw new Error('분배 요청 처리 중 오류가 발생했습니다.');
        }
    }
};
