...
function DistributeRequestModal({ isOpen, onClose, projectId, title }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!distributionAmount.trim()) {
            toast.error("분배 금액을 입력해주세요.");
            return;
        }

        if (isNaN(parseFloat(distributionAmount)) || parseFloat(distributionAmount) <= 0) {
            toast.error("올바른 분배 금액을 입력해주세요.");
            return;
        }
    ...
    const isFormValid = reason.trim() && distributionAmount.trim() && !isNaN(parseFloat(distributionAmount)) && parseFloat(distributionAmount) > 0 && files.length > 0;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-gray-800">프로젝트 분배 요청</h2>
                    <button
                ...
                <div className="mb-6 p-3 bg-gray-50 rounded">
                    ...
                    <p className="text-xs text-gray-500">프로젝트 ID: {projectId}</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                            분배 금액 (원) <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="number"
                            value={distributionAmount}
                            onChange={(e) => setDistributionAmount(e.target.value)}
                            placeholder="분배할 금액을 입력해주세요"
                ...
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
                    <p className="text-xs text-blue-800">
                        분배 요청 후에는 취소할 수 없으며, 관리자 승인 시 투자자들에게 수익이 분배됩니다.
                    </p>
                </div>
            </div>
        </div>
...
export default DistributeRequestModal;
