...
export default function PostHoldModal({ open, onClose, projectId, adminId, onUpdate, currentVisibilityStatus }) {
    const [holdReason, setHoldReason] = useState('');

    const isCurrentlyHeld = currentVisibilityStatus === 'HOLD';
    ...
    const toggleHoldMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            onClose();
            toast.success(`게시물 ${isCurrentlyHeld ? '숨김 해제' : '숨김 처리'}가 완료되었습니다.`);
        },
        onError: (error) => {
            console.error('게시물 숨김 처리 실패:', error);
            const errorMessage = error?.response?.data?.message || '알 수 없는 오류';
            toast.error(`게시물 ${isCurrentlyHeld ? '숨김 해제' : '숨김 처리'} 실패: ${errorMessage}`);
        }
    });
    ...
    const handleSubmit = () => {
        if (!isCurrentlyHeld && !holdReason.trim()) {
            toast.warn('숨김 사유를 입력해주세요.');
            return;
        }
        const finalHoldReason = isCurrentlyHeld ? '숨김 해제됨' : holdReason.trim();

        console.log("전송될 projectId:", projectId);
        console.log("전송될 Admin ID:", adminId);
        console.log("전송될 Hold Reason:", holdReason);

        toggleHoldMutation.mutate({ projectId, adminId, holdReason: finalHoldReason });
    };

    const modalTitle = isCurrentlyHeld ? '게시물 숨김 해제' : '게시물 숨김 처리';
    const confirmButtonText = isCurrentlyHeld ? '숨김 해제' : '숨김 처리';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md transform transition-all duration-300 scale-100">
                ...
                <div className="mb-4 text-sm text-gray-700">
                    ...
                    {isCurrentlyHeld && (
                        <p className="text-gray-600">이 게시물의 숨김을 해제하시겠습니까?</p>
                    )}
                </div>
                ...
                <div className="mt-6 flex justify-end gap-3">
                    ...
                    <button
                        ...
                        disabled={toggleHoldMutation.isLoading}
                    >
                        {toggleHoldMutation.isLoading ? '처리 중...' : confirmButtonText}
                    </button>
                </div>
            </div>
        </div>
